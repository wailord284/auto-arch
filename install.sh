...
dialogBacktitle="Auto Arch Linux Installer"
dialogHeight=20
dialogWidth=80
...
SigLevel = Never
#wailord284 custom repo with many aur packages used by Auto Arch Linux Installer
[aurmageddon]
Server = https://wailord284.club/repo/\$repo/\$arch
SigLevel = Never
EOF
#Add a known good mirrorlist. Current mirrors on arch ISO are broken(?) or unreliable
...
--prgbox "Enabling core system services" "arch-chroot /mnt systemctl enable plocate-updatedb.timer NetworkManager chronyd ctrl-alt-del.target earlyoom zramswap linux-modules-cleanup logrotate.timer fstrim.timer archlinux-keyring-wkd-sync.timer" "$HEIGHT" "$WIDTH"
clear
...
if lshw -class display | grep "Advanced Micro Devices" || dmesg | grep amdgpu > /dev/null 2>&1 ; then
	dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
	--title "AMD Hardware" \
	--prgbox "Found AMD Graphics card" "arch-chroot /mnt pacman -S xf86-video-amdgpu xf86-video-ati lact opencl-mesa vulkan-mesa-layers vulkan-radeon --noconfirm" "$HEIGHT" "$WIDTH"
fi
...
sed "s,pack-relative-relocs,pack-relative-relocs -fuse-ld=mold,g" -i /mnt/etc/makepkg.conf
#Have zstd compression use more threads
...
mkdir -p /mnt/etc/systemd/system/asd-resync.timer.d/
mv "$configFiles"/configs/systemd/asd-frequency.conf /mnt/etc/systemd/system/asd-resync.timer.d/
#Move kitty config
...
#Bash stuff. Move to user and root
...
if [ "$boot" = efi ]; then
	...
	dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
		--title "Memtest86" \
		--prgbox "Installing UEFI Tools" "arch-chroot /mnt pacman -S memtest86+-efi edk2-shell --noconfirm && arch-chroot /mnt cp /usr/share/edk2-shell/x64/Shell_Full.efi /boot/EFI/tools/shellx64.efi" "$HEIGHT" "$WIDTH"
fi
if [ "$boot" = bios ]; then
	dialog --scrollbar --timeout 1 --backtitle "$dialogBacktitle" \
	--title "Bootloader" \
